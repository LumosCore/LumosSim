...
def solve(spine_num_per_pod, spine_up_port_num, c_ijt: np.ndarray, u_ijkt: np.ndarray, is_itv = True, releax =False, is_dragon_fly=False):
    ...
    t_ij_dis = np.empty((pod_num, pod_num), dtype=pywraplp.Variable)
    for i in range(pod_num):
        for j in range(pod_num):
            t_ij_dis[i, j] = solver.IntVar(-spine_up_port_num*100*spine_num_per_pod, spine_up_port_num*100*spine_num_per_pod, f't_{i}_{j}')
    ...
    if not releax:
        ...
        for i in range(pod_num):
            for j in range(pod_num):
                for t in range(spine_num_per_pod):
                    solver.Add(c_ijt[i, j, t] <=  solver.Sum(x_ijkt[i, j, :, t].ravel().tolist()))
    # 1.5 对称
    if not is_itv:
        for i in range(pod_num):
            for j in range(pod_num):
    ...
    # 设置目标函数最小化
    obj = solver.Sum(h_ijkt.ravel().tolist()) + 1000*solver.Sum(t_ij_dis.ravel().tolist()) + 1000*solver.Sum(t_jt_dis2.ravel().tolist()) + 1000*solver.Sum(t_it_dis2.ravel().tolist())
    solver.Minimize(obj)

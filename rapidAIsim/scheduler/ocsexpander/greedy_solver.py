...
class GreedySolver:
    ...
    def __init__(self, pod_num, spine_num_per_pod, oxc_list, original_oxc_to_each_spine_num, c_ijt, u_ijkt,
        ...
        # print("debug c_ijt")
        # print(c_ijt)
        self.oxc_list = [oxc_list[i] for i in range(len(oxc_list))]
        ...
        self.oxc_physical_group_size = self.port_per_spine
    ...
    def solve(self):
        ...
        for t in range(self.spine_num_per_pod):
            for logical_local_oxc_id in range(self.oxc_physical_group_size):
                ...
                self.spine_connected_oxc_list_map[t].append(1 * logical_global_oxc_id)
        tmp_x_ijkt = self.solve_merge_and_decomp(oxc_groups, self.c_ijt, self.u_ijkt, 0)
        x_ijkt = np.zeros((self.pod_num, self.pod_num, self.oxc_physical_group_size, self.spine_num_per_pod),
                          dtype=int)
        x_ijkt = tmp_x_ijkt
        ...
        for i in range(self.pod_num):
            for j in range(self.pod_num):
                for k in range(self.oxc_physical_group_size):
                    for t in range(self.spine_num_per_pod):
                        if x_ijkt[i, j, k, t] > x_ijkt[j, i, k, t]:
                            # x_ijkt[i, j, k, t] = max(0,x_ijkt[j, i, k, t])
                            flag = True
        if flag:
            print("l2_incomp")
        ...
        for i in range(pod_num):
            for t in range(spine_num_per_pod):
                for k in cur_oxc_list:
                    ...
                    # print("debug logical_spine_oxc_link[i, t, oxc_to_group_map[k]]")
                    # print(k,self.spine_connected_oxc_list_map[t])
                    # print(logical_spine_oxc_link[i, t, oxc_to_group_map[k]])
                    # print(original_oxc_to_each_spine_num)

...
def get_dir(props, is_test, data_dir=None):
    """Get the train or test directory for the given topology."""
    postfix = "test" if is_test else "train"
    base_dir = data_dir if data_dir else DATA_DIR
    return os.path.join(base_dir, props.topo_name, postfix)
...
def get_train_test_files(props, data_dir=None):
    """Get the train and test files for the given properties."""
    train_dir = get_dir(props, False, data_dir)
    test_dir = get_dir(props, True, data_dir)

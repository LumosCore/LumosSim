...
class Routing:
    ...
    def MLU_traffic_engineering(self, demands):
        ...
        """
        demands = demands.reshape((1, self.topology.number_of_nodes(), self.topology.number_of_nodes()))
        ...
        m.addConstrs(
            ...
            for i in range(self.topology.number_of_nodes())
            for j in range(self.topology.number_of_nodes())
            if j != i and len(self.candidate_path[(i, j)]) > 0
        )
    ...
    def dual_cope_traffic_engineering(self, demands, predict_dms, beta):
        predict_dms = predict_dms.reshape((predict_dms.shape[0], self.topology.number_of_nodes(), self.topology.number_of_nodes()))
    ...
    def max_mean_mlu_traffic_engineering(self, demands, beta=1):
        """Compute the traffic engineering solutions for multiple demands to minimize the worst-case MLU.

        Args:
            demands: the traffic demands, shape: (demand_number, number_of_nodes * number_of_nodes)
            beta: the weight of the ALU
        """
        demands = demands.reshape((1, self.topology.number_of_nodes(), self.topology.number_of_nodes()))
        m = grb.Model('traffic_engineering_grb')
        m.Params.OutputFlag = 0
        mlu = m.addVar(lb=0, vtype=grb.GRB.CONTINUOUS, name='mlu')
        link_utilization = [f'lu_{i}' for i in range(len(self.topology.edges))]
        name_path_weight = [f'w_{i}_{j}_{k}'
                            for i in range(self.topology.number_of_nodes())
                            for j in range(self.topology.number_of_nodes())
                            if j != i
        ...
        lu = m.addVars(link_utilization, lb=0, vtype=grb.GRB.CONTINUOUS, name='link_utilization')
        # the sum of the routing weights of the candidate paths for each s-d pair should be 1
        m.addConstrs(
            grb.quicksum(
                path_weight[f'w_{i}_{j}_{k}'] for k in range(len(self.candidate_path[(i, j)]))
        ...
        for demand in demands:
            ...
            m.addConstrs(
                grb.quicksum(
                    path_weight[f'w_{src}_{dst}_{k}'] * demand[src][dst] / self.topology.edges[edge]['capacity'] 
                    for (src, dst, k) in self.edge_to_path[edge]
                ) == lu[f'lu_{i}']
                for i, edge in enumerate(self.topology.edges)
            )

        obj = m.addVar(lb=0, vtype=grb.GRB.CONTINUOUS, name='obj')
        m.addConstr(obj >= mlu + grb.quicksum(lu) / len(link_utilization) * beta)
